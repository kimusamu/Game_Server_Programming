	...
	while (true)
	{
		...
		case OP_NPC_MOVE:
		{
			...
			if (npc->_greet_mode)
			{
				...
				{
					std::lock_guard<std::mutex> lk(clients[npc_id]->_ll);
					lua_State* L = clients[npc_id]->_L;

					lua_getglobal(L, "API_SendMessage");

					lua_pushinteger(L, npc_id);
					lua_pushinteger(L, clients[npc_id]->_greet_target);
					lua_pushstring(L, "BYE");

					if (lua_pcall(L, 3, 0, 0) != LUA_OK)
					{
						const char* err = lua_tostring(L, -1);
						std::cerr << "[Lua Error] API_SendMessage: " << err << "\n";
						lua_pop(L, 1);
					}
