...
int get_new_client_id()
{
	for (int i = 0; i < MAX_USER; ++i) 
	{
		...
		lock_guard <mutex> lk(it->second._s_lock);
...
void process_packet(int c_id, char* packet)
{
	...
	case CS_LOGIN: 
	{
		...
		{
			{
				lock_guard<mutex> ll(s._s_lock);

				if (ST_INGAME != s._state) 
				{
					continue;
				}
			}
			...
			s.send_add_player_packet(c_id);
			clients[c_id].send_add_player_packet(s._id);
		}
		break;
	}
	...
	case CS_MOVE: 
	{
		...
		for (auto& [id, s] : clients) 
		{
			if (s._state != ST_INGAME)
			{
				continue;
			}

			s.send_move_packet(c_id);
...
void disconnect(int c_id)
{
	for (auto& [id, s] : clients) 
	{
		...
		s.send_remove_player_packet(c_id);
	}
...
int main()
{
	...
	clients.max_load_factor(1.0f);

	for (int i = 0; i < MAX_USER; ++i) 
	{
		clients.try_emplace(i);
	}
