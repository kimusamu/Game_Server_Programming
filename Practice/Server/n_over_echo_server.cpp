...
private:
	...
	void do_recv()
	{
		...
		ZeroMemory(&_recv_over, sizeof(_recv_over)); // 재사용 할 때마다 세탁 실시
...
public:
	...
	SESSION(long long session_id, SOCKET s) : _id(session_id), _c_socket(s) 
	{
		...
		_recv_wsabuf[0].buf = _recv_buffer; // 한번 세팅시 변경 안됨
...
int main()
{
	...
	while (true)
	{
		...
		//g_users[client_id] = new_session; new_session이 생성 및 소멸, 그러나 이런 경우 session이 두번 생성됨 -> recv가 두번
